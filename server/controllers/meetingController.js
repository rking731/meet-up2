        ...
        if(userPlan === "free"){
            ...
            if(monthlyCount >= 70){
                return res.status(403).json({
                    error: "Monthly limit reached. Free plan includes 70 meetings per month. Please upgrade to premium for unlimited meetings!",
                    limitReached: true,
                    monthlyCount,
                    limit: 70,
                })
...
export const getSessionDetails = async (req, res)=>{
     try {
        const {id} = req.params;
        const userId = req.user.id;
        const meetings = await sql`SELECT
        m.*, u.id AS host_user_id, u.name AS host_name, u.email AS host_email
        FROM meetings m JOIN users u ON m.host_id = u.id WHERE m.meeting_id= ${id}`;
        ...
        const participants = await sql` SELECT mp.*, u.email
               FROM meeting_participants mp 
               LEFT JOIN users u ON mp.user_id = u.id 
               WHERE mp.meeting_id = ${m.id}`;
     ...
     try {
        const userId = req.user.id;
          ...
          const monthlyCountResult = await sql`
            ...
            const monthlyLimit = plan === "premium" ? null : 70;
            
            res.json({
                plan, monthlyCount, monthlyLimit, maxParticipants: plan === "premium" ? 120 : 30,
            })
